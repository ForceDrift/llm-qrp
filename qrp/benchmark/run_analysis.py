...
class SLEDEntropyAnalyzer:
    ...
    def _compute_sled(self, all_logits_cpu, input_ids):

        layers_to_test = list(range(self.sled_model.layers))

        # SLED + KL share a single forward pass whose logits are computed on CPU

        # to keep GPU memory bounded (split-phase OOM safety).

        disagreement_results = self.sled_model.layer_disagreement_from_cache(

            all_logits_cpu, input_ids, evolution_scale=5,
    ...
    def _compute_kl(self, all_logits_cpu):

        kl_prev = self.sled_model.kl_between_current_prev_from_cache(all_logits_cpu)
    ...
    def run(self, prompt):
        ...
        if "sled" in self.signals or "kl" in self.signals:

            all_logits_cpu, input_ids = self.sled_model.compute_layer_logits_to_cpu(prompt)

            if "sled" in self.signals:

                signal_values["sled"] = self._compute_sled(all_logits_cpu, input_ids)

            if "kl" in self.signals:

                signal_values["kl"] = self._compute_kl(all_logits_cpu)

            del all_logits_cpu, input_ids
...
def _merge_model_level_scores(root_dir, aggregated_file):

    """Merge per-dataset layer scores into a model-level aggregated_scores.json."""

    all_layer_scores = defaultdict(list)

    for ds_dir in os.listdir(root_dir):

        ds_result = os.path.join(root_dir, ds_dir, "results.json")

        if os.path.isfile(ds_result):

            with open(ds_result, "r") as f:
            ...
            for item in results:
                ...
                for layer_key, score in analysis.items():
                    ...
                    if not math.isnan(score) and not math.isinf(score):
                        ...
                        all_layer_scores[layer_key].append(score)

    merged_scores = {k: sum(v) / len(v) for k, v in all_layer_scores.items() if v}
...
if __name__ == "__main__":
    ...
    # Also merge across all datasets into a model-level aggregated_scores.json

    root_dir = os.path.join(args.output_folder, model_name_safe)

    model_aggregated_file = os.path.join(root_dir, "aggregated_scores.json")

    merged = _merge_model_level_scores(root_dir, model_aggregated_file)

    print(f"Model aggregated scores (all datasets) saved to: {model_aggregated_file}")
