...
def generate_report(results_dir, output_dir):
    ...
    # Traverse the results directory for JSON files
            ...
            if file.endswith(".json"):
                ...
                try:
                    with open(path, "r") as f:
                        ...
                        # Case 1: Unified Benchmark Results
                        if "decoding" in res:
                        ...
                        # Case 2: Quantization Sweep Results
    ...
    if quant_data:
        ...
        with open(os.path.join(output_dir, "quantization_summary.md"), "w") as f:
            ...
            f.write(md_quant)
            f.write("\n")
        print(f"Quantization summary saved.")

        # Graphics for quantization (Accuracy vs Threshold)
        ...
        for ds in datasets:
            ...
            # Baseline line
            baseline_acc = q_df[(q_df["Dataset"] == ds) & (q_df["Condition"] == "baseline")]["Accuracy"].iloc[0]
